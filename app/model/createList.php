<?php 
    ...
    if(isset($_POST['list'])){
        ...
        if(!in_array($fileExt, $allowed)){
            if($fileSize > 1000000){
                header("location:?page=createList&error=fileTooBig");
                exit();
            }
            header("location:?page=createList&error=fileExtNotSupp");
            exit();

        }elseif(empty($prodName) || empty($prodPrice) || empty($prodDescription) || empty($prodPic) ||  $prodCategorie == "nothing"){
            header("location:?page=createList&error=emptyFields");    //checking if a field is empty
            exit();
            ...
            //now every entry is valid                   
                    ...
                    $sql = "INSERT INTO products (prodName,prodPrice,prodDescription,ownerId,pic,categorie) VALUES (?,?,?,?,?,?) ";
                    $stmt = mysqli_stmt_init($conn);

                    if(!mysqli_stmt_prepare($stmt, $sql)){
                        header("location:?page=createList&error=sqlError"); 
                        exit();
                    }else{
                        mysqli_stmt_bind_param($stmt, "ssssss", $prodName, $prodPrice, $prodDescription, $userId, $fileId, $prodCategorie); 
                        mysqli_stmt_execute($stmt);
                        header("location:?page=listings&reg=itemListed");   //sending the user back with a success message 
                        exit();
                    }
